...
function logger(handler) {
  return req => {
    ...
    let requestId = Stdlib_Option.getOr(Primitive_option.fromNullable(req.headers.get("x-request-id")), "unknown");
    return handler(req).then(response => {
      let duration = Date.now() - start;
      let status = response.status.toString();
      console.log(`[` + requestId + `] ` + method + ` ` + path + ` ` + status + ` ` + duration.toString() + `ms`);
      return response;
    });
  };
}
